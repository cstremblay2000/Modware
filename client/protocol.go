...
/**
 * description:
 *	This is the packet used by the KeyServer and is sent to 
 *	the ModwareClient in the verifying host protocol flow
 *	this packet comprises of the public key of the ModwareServer,
 *	the Challenge generated by the keyserver,
 *	the expected signature of the challenge form the ModwareServer
 *	The expected signature signed by the KeyServer
 */
 ...
 *	the gob encoded byte array
 */
 func VerifyHostExpectedResultsToBytes( packetStruct VerifyHostExpectedResults ) ( []byte, error ) {
	buf := new(bytes.Buffer)
    enc := gob.NewEncoder(buf)
    err := enc.Encode(packetStruct)
    if err != nil {
 ...
 *	the EncapsulatedModbusPacket struct
 */
func VerifyHostExpectedResultsFromBytes(b []byte) (*VerifyHostExpectedResults, error) {
    buf := bytes.NewBuffer(b)
    dec := gob.NewDecoder(buf)
    em := &VerifyHostExpectedResults{}
    err := dec.Decode(em)
    if err != nil {
        return nil, err
    }
